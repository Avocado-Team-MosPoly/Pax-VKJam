...
public class TeamRoundManager : RoundManager
{
    ...
    protected override void WinRound()
    {
        ...
        tokensToAdd += GameManager.Instance.IsDangerousCard ? config.BonusIfMonsterGuessed_TM_DM.GetValue(playersCount) * playersCount : config.BonusIfMonsterGuessed_TM_MM.GetValue(playersCount) * playersCount; // calculate bonus for monster type
